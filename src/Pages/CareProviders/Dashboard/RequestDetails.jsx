...
import { fetchRequestById, postReview } from '../../../Redux/CareProviderRequest'
...
function RequestDetails() {
  ...
  const { current } = useSelector(s => s.careProviderRequests || { current: null })
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex-1 font-sfpro px-8 py-8 ml-64">
        ...
        <div className="flex items-center mb-6">
          <img src={current && current.seeker ? resolveImage(current.seeker.profile_image_url) : 'https://ui-avatars.com/api/?name=User&background=E5E7EB&color=374151&size=64'} alt={current && current.seeker ? current.seeker.full_name : 'User'} className="w-16 h-16 rounded-full object-cover mr-6" />
          <div>
            <div className="text-xl font-semibold text-gray-800">{current ? (current.seeker ? current.seeker.full_name : (current.job_title || '')) : 'Loading...'}</div>
            <div className="text-gray-500 text-sm mt-1">{current ? (current.date_range_for_task || '') : ''}</div>
          </div>
        </div>
        <div className="flex gap-6 mb-8">
          ...
          <div className="bg-white border border-gray-100 rounded-lg px-6 py-4 flex flex-col items-start min-w-[120px]">
            ...
            <span className="text-gray-800 font-semibold text-lg flex items-center gap-2">5.0 {Array.from({ length: 5 }).map((_, i) => <FaStar key={i} className="text-[#cb9e49] text-base" />)}</span>
          </div>
        </div>
        <div className="mb-8">
        ...
        <div className="mb-8">
          ...
          <div className="bg-white border border-gray-100 rounded-lg px-6 py-4 text-gray-700 text-base">
            <div className="mb-2"><strong>Title:</strong> {current ? (current.job_title || '') : ''}</div>
            <div className="mb-2"><strong>Location:</strong> {current ? (current.seeker_location || '') : ''}</div>
            <div className="mb-2"><strong>Seeker since:</strong> {current ? (current.seeker_join_date || '') : ''}</div>
            <div className="mb-2"><strong>Task date range:</strong> {current ? (current.date_range_for_task || '') : ''}</div>
            <div className="mb-2"><strong>Average rating:</strong> {current ? (current.seeker_average_rating || '') : ''}</div>
            <div className="mb-2"><strong>Rate per hour:</strong> {current ? (current.rate_per_hour ? `$${current.rate_per_hour}` : '') : ''}</div>
            <div className="mb-2"><strong>Review from seeker:</strong> {current ? (current.review_from_seeker || 'No review') : ''}</div>
          </div>
        </div>
        <button
          ...
          onClick={async () => {
            ...
            setSubmitting(true)
            try {
              const res = await dispatch(postReview({ booking_id, rating: ratingValue, comment }))
