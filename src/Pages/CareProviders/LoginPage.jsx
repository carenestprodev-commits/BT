  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.payload && result.payload.access) {
        ...
        const userType = userData?.user_type || "provider";

        const isStaff = userData?.is_staff || result.payload?.is_staff;

        if (userType === "provider" || isStaff) {
          ...
          // wrong portal: clear auth and show message
          try {
            dispatch(logout());
          } catch {
            // best-effort cleanup
          ...
          setError(
            "You are not a care provider — please login through the Care Seeker portal",
          );
        }
      } else {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-white">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-10 w-full max-w-md">
        ...
        <form className="space-y-4 font-sfpro" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-[#0093d1] text-white font-medium py-2 rounded-md hover:bg-[#007bb0] transition disabled:opacity-60 flex items-center justify-center"
