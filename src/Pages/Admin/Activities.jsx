...
import { fetchActivitiesStats, fetchAllActivities, fetchActivityById, deleteActivity } from '../../Redux/AdminActivities'
...
function Activities() {
  ...
  const { stats, activities, currentActivity } = useSelector(s => s.adminActivities || { stats: {}, activities: [], currentActivity: null })
  ...
  useEffect(() => {
    if (Array.isArray(activities)) {
      const mapped = activities.map(a => ({
        ...
        status: a.status === 'pending_approval' ? 'Pending' : a.status === 'ongoing_activity' ? 'Ongoing Activity' : (a.status === 'fulfilled' || a.status === 'completed') ? 'Fulfilled' : a.status,
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      data = data.filter(r => String(r.id).toLowerCase().includes(q) || r.name.toLowerCase().includes(q));
    }
  ...
  return (
    <div className="p-6 text-black bg-white">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { key: 'all', label: 'All Activities', value: stats?.all_activities ?? 0, icon: CubeIcon },
          { key: 'fulfilled', label: 'Fulfilled Requests', value: stats?.fulfilled_requests ?? 0, icon: CubeIconGreen },
          { key: 'pending', label: 'Pending Approval', value: stats?.pending_approval ?? 0, icon: CubeIconPink },
          { key: 'ongoing', label: 'Ongoing Activities', value: stats?.ongoing_activities ?? 0, icon: CubeIconOrange },
        ].map(s => {
        ...
        {alert && (
          <div className={`mb-4 px-4 py-3 rounded-md ${alert.type === 'success' ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`} role="alert">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => { setAlert(null); if (alertTimer.current) { clearTimeout(alertTimer.current); alertTimer.current = null } }} className="text-sm font-medium">Close</button>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-24">
          ...
          <div className="relative bg-white w-[380px] rounded-lg shadow-lg p-6 z-50">
            ...
            <div className="space-y-3 text-sm text-slate-700">
              ...
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Care Provider&apos;s Name</span><span className="text-right">{editRow.careProviderName || 'Ezeonu Justina'}</span></div>
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Care Provider&apos;s Phone Number</span><span className="text-right">{editRow.careProviderPhone || '+234123456789'}</span></div>
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Care Provider&apos;s Email</span><span className="text-right">{editRow.careProviderEmail || 'olivia@untitledui.com'}</span></div>
            ...
            <div className="flex items-center justify-center gap-3">
              <button className="px-4 py-2 bg-red-600 text-white rounded-md" onClick={async () => {
                try {
                  ...
                  if (alertTimer.current) { clearTimeout(alertTimer.current); alertTimer.current = null }
                  setAlert({ type: 'error', text: 'Failed to delete activity' })
                  alertTimer.current = setTimeout(() => setAlert(null), 3000)
                }
              }}>Delete</button>
