...
function Earnings() {
  ...
  const filtered = useMemo(() => {
    let data = [...rows];
    // stat filter
    if (activeStat === 'careSeekers') data = data.filter(r => r.source === 'careSeekers');
    if (activeStat === 'platform') data = data.filter(r => r.source === 'platform');
    if (q.trim()) {
      const t = q.toLowerCase();
      data = data.filter(r => r.id.toLowerCase().includes(t) || r.user.toLowerCase().includes(t));
    }
    if (date) {
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1">
          <div className="flex items-center bg-white rounded-md px-3 py-2 shadow-sm text-black">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="search for transaction" className="outline-none w-full text-sm bg-white text-black" />
          </div>
        </div>
      ...
      <div className="bg-white rounded-md shadow-sm overflow-hidden text-black">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="p-6 text-center text-slate-400">No results</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-24">
          ...
          <div className="relative bg-white w-[340px] rounded-lg shadow-lg p-6 z-50 max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto text-sm text-slate-700 space-y-3 pr-2">
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Transaction ID</span><span className="text-right">{detailRow.id}</span></div>
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">User Name</span><span className="text-right">{detailRow.user}</span></div>
              ...
              <div className="flex justify-between py-2 border-b"><span className="text-slate-500">Time</span><span className="text-right">{detailRow.time}</span></div>
              <div className="flex justify-between py-2"><span className="text-slate-500">Date</span><span className="text-right">{detailRow.date}</span></div>
            </div>
            <div className="mt-4">
              <button className="w-full bg-[#0b93c6] text-white py-2 rounded-md mb-3" onClick={() => downloadRowCSV(detailRow)}>Download</button>
              <button className="w-full border border-slate-200 text-slate-700 py-2 rounded-md" onClick={() => setDetailRow(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
