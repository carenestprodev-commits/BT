...
function Subscription() {
  ...
  const { subscriptions, current } = useSelector(
    (s) => s.adminSubscription || { subscriptions: [], current: null }
  );
    ...
    if (Array.isArray(subscriptions)) {
      const mapped = subscriptions.map((s) => ({
        ...
        amount: s.amount
          ...
          : "0.00",
        start: s.start_date
          ? dayjs(s.start_date).format("DD-MM-YYYY")
          : s.start
          ? dayjs(s.start).format("DD-MM-YYYY")
          : "",
        end: s.end_date
          ? dayjs(s.end_date).format("DD-MM-YYYY")
          : s.end
          ? dayjs(s.end).format("DD-MM-YYYY")
          : "",
        raw: s,
      }));
      setRows(mapped);
  ...
  const filtered = useMemo(() => {
    ...
    if (date)
      data = data.filter((r) => r.start === dayjs(date).format("DD-MM-YYYY"));
    return data;
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 text-black rounded-md text-sm border ${
                    isActive ? "bg-[#0ea5d7] text-white" : "bg-white text-black"
                  } hover:shadow-sm`}
                >
                  {pbtn}
                </button>
          ...
          <div className="relative bg-white w-[340px] rounded-lg shadow-lg p-6 z-50 max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto text-sm text-slate-700 space-y-3 pr-2">
              ...
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="text-right">
                  {current.plan || current.subscription_plan || "Unknown"}
                </span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-slate-500">Amount</span>
                <span className="text-right">
                  {current.amount ?? current.price ?? "0.00"}
                </span>
              </div>
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="text-right">
                  {current.start_date
                    ? dayjs(current.start_date).format("DD-MM-YYYY")
                    : current.start
                    ? dayjs(current.start).format("DD-MM-YYYY")
                    : ""}
                </span>
              </div>
              <div className="flex justify-between py-2">
                ...
                <span className="text-right">
                  {current.end_date
                    ? dayjs(current.end_date).format("DD-MM-YYYY")
                    : current.end
                    ? dayjs(current.end).format("DD-MM-YYYY")
                    : ""}
                </span>
              </div>
            </div>
