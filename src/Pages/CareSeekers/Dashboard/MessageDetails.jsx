  ...
  return (
    <div className="flex min-h-screen bg-white font-sfpro">
      ...
      <div className="flex-1 font-sfpro px-8 py-8 md:ml-64">
        <button
          className="mb-8 text-gray-500 hover:text-gray-700 text-2xl font-bold flex items-center gap-2"
        ...
        >
          ...
          <div>
            <div className="text-xl font-semibold text-gray-800">
              {details?.user?.full_name || "Provider"}
            </div>
        ...
        <div className="mb-8">
          ...
          details.testimonials.length > 0 ? (
            details.testimonials.map((t) => (
              <div
                key={t.id}
                className="bg-white border border-gray-100 rounded-lg px-6 py-4 text-gray-700 text-base mb-3"
              >
        ...
        <button
          ...
          onClick={async () => {
            ...
            const res = await dispatch(
              postProviderReview({ booking_id, rating, comment: feedback })
            );
            if (res.error) {
              alert(
                "Failed: " +
